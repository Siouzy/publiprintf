...
/*   Updated: 2018/02/23 18:15:25 by srepelli    ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
int		ft_p(t_spec *sp, intmax_t arg)
{
	...
	{
		sp->indic = 'x';
		sp->attrib = ft_strjoin(sp->attrib, "#");
		ft_strdel(&sp->taille);
...
int		ft_d(t_spec *sp, intmax_t arg)
{
	...
	if (!arg && sp->isprec && !sp->precis)
		tab[0] = tab[0] - 1;
	else if (arg < 0 && (((ft_countchar(sp->attrib, '0') &&
		!ft_countchar(sp->attrib, '-') && tab[1] < sp->largmin)) ||
		(tab[1] <= sp->precis)))
		ft_putnbr(-arg);
...
int		ft_o(t_spec *sp, intmax_t arg)
{
	...
	if (!arg && sp->isprec && !sp->precis && !ft_countchar(sp->attrib, '#'))
		i = (sp->largmin && ft_countchar(sp->attrib, '-') ? i - 2 : i - 1);
	else
		write(1, s, len);
	if (sp->largmin && ft_countchar(sp->attrib, '-'))
