...
int		display_signs_zero(t_spec *sp, int len, intmax_t arg, int j)
{
	...
	if ((sp->indic == 'd' || sp->indic == 'i') &&
			ft_countchar(sp->attrib, '+') && arg >= 0 && len < sp->largmin)
	{
		write(1, "+", 1);
		j++;
	}
...
int		ft_zero(t_spec *sp, intmax_t arg, int len)
{
	...
	j = 0;
	k = 0;
	if (ft_countchar(sp->attrib, ' '))
		k = 1;
	if (!ft_countchar("diouxX%", sp->indic) ||
			(sp->isprec && sp->indic != '%') || ft_countchar(sp->attrib, '-'))
		return (0);
	if (ft_countchar(sp->attrib, '#') &&
			(sp->indic == 'x' || sp->indic == 'X') && arg != 0)
		j = 2;
	if (ft_countchar(sp->attrib, '#') &&
			(sp->indic == 'o' && arg != 0))
		len++;
